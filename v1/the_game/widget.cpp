...
void Widget::keyPressEvent(QKeyEvent* event){
    ...
    if(event->key() == Qt::Key_R && gameSceneLevel == Widget::LevelPause){
        gameExeRestart();
        gameSceneLevel = Widget::LevelGaming;
    }
    if(event->key() == Qt::Key_Backspace && gameSceneLevel == Widget::LevelPause){
        gameExeStop();
...
void Widget::gameExeOver(){
    ...
    // gameScene_gaming.addItem(&gameOver_0);
    // timerOFF();
    // _timerOFF();
...
void Widget::gameExeWin(){
    // gameScene_gaming.addItem(&gameWin_0);
    // timerOFF();
    // _timerOFF();
...
void Widget::propEffect(int _type){
    switch(_type){
        ...
        case gameProp::extraLife:{
            ...
            else {playerShip_0->shipLife += 1;/*renew*/}
            break;
        }
